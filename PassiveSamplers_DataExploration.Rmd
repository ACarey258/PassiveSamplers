...
ggplot(data = Sthd, mapping = aes(x = d13C_UW, y = PBDEsLipNorm)) +
  geom_point(mapping = aes(color = Year), size = 2, shape = 1) +
  geom_smooth(method = lm, se = TRUE, level = 0.95) +
  geom_text(aes(label = ifelse(PBDEsLipNorm > 1000, #only labels points above a certain threshold
                               as.character(SampleID),
                               ' ')),
            hjust = 0,
            vjust = 0)
...
```{r}
Sthd$labyn <- ifelse(Sthd$PBDEsLipNorm > 900, Sthd$SampleID, "")

ggplot(data = Sthd, mapping = aes(x = d15N_UW, y = PBDEsLipNorm)) +
  geom_point(mapping = aes(color = Year), size = 2, shape = 1) +
  geom_smooth(method = lm, se = TRUE, level = 0.95) +
  geom_label_repel(aes(label = Sthd$labyn,
                    fill = factor(Year)), color = 'white',
                    size = 3.5)
```
